Public Class Intensity_Process
    ...
    Private Sub Intensity_Process()
        ...
        For x As Integer = 0 To _img.Height - 1
            For y As Integer = 0 To _img.Width - 1
                ...
                ave = CInt((CInt(Intensity) + CInt(Intensity) + CInt(Intensity)) / 3)

                _arrayofBW(x, y) = ave
                _arrayofRed(x, y) = CInt(Intensity)
                _arrayofGreen(x, y) = CInt(Intensity)
                _arrayofBlue(x, y) = CInt(Intensity)

                _pixelBW &= CInt((CInt(ave) + CInt(ave) + CInt(ave)) / 3) & vbTab
                _pixelRED &= CInt(Intensity) & vbTab
                _pixelGREEN &= CInt(Intensity) & vbTab
                _pixelBLUE &= CInt(Intensity) & vbTab

                img_Original.SetPixel(x, y, Color.FromArgb(255, Intensity, Intensity, Intensity))
                img_BW.SetPixel(x, y, Color.FromArgb(255, ave, ave, ave))
                img_R.SetPixel(x, y, Color.FromArgb(255, Intensity, 0, 0))
                img_G.SetPixel(x, y, Color.FromArgb(255, 0, Intensity, 0))
                img_B.SetPixel(x, y, Color.FromArgb(255, 0, 0, Intensity))
